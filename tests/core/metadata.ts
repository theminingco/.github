...
describe("metadata", () => {
  ...
  it("Should unpack metadata with allocation", () => {
    const metadata = { name: "name", symbol: "symbol", description: "description", image: "image", external_url: "external_url", allocation: [{ symbol: "symbol", amount: "100%" }] };
    const result = unpackMetadata(metadata);
    assert.strictEqual(result.name, "name");
    assert.strictEqual(result.symbol, "symbol");
    assert.strictEqual(result.description, "description");
    ...
    assert.strictEqual(result.external_url, "external_url");
    assert.deepStrictEqual(result.attributes, []);
    assert.deepStrictEqual(result.allocation, [{ symbol: "symbol", amount: "100%" }]);
    assert.strictEqual(result.attribution, "");
  });
  ...
  it("Should fail if allocation is missing symbol", () => {
    const metadata = { name: "name", symbol: "symbol", description: "description", image: "image", external_url: "external_url", allocation: [{ amount: "100%" }] };
    assert.throws(() => unpackMetadata(metadata));
  });

  it("Should fail if symbol is not a string", () => {
    const metadata = { name: "name", symbol: "symbol", description: "description", image: "image", external_url: "external_url", allocation: [{ symbol: 1, amount: "100%" }] };
    assert.throws(() => unpackMetadata(metadata));
  });
  ...
  it("Should fail if percentage is not a string", () => {
    const metadata = { name: "name", symbol: "symbol", description: "description", image: "image", external_url: "external_url", allocation: [{ symbol: "symbol", amount: 1 }] };
    assert.throws(() => unpackMetadata(metadata));
  });

  it("Should fail if allocation has extra keys", () => {
    const metadata = { name: "name", symbol: "symbol", description: "description", image: "image", external_url: "external_url", allocation: [{ symbol: "symbol", amount: "100%", extra: "extra" }] };
    assert.throws(() => unpackMetadata(metadata));
  });
